...
class Database
{
    ...
    public function contructQuery()
    {
        ...
        if ($this->queries['limit'] && is_array($this->queries['limit']) && count($this->queries['limit'])) {
            $query .= $this->queries['limit'];
        }
        if ($this->queries['offset'] && is_array($this->queries['offset'] )&& count($this->queries['offset']) && $this->_typeQuery == 'SELECT') {
